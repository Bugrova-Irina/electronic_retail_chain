...
class SellerAdmin(admin.ModelAdmin):
    list_display = [
        "seller_title", "seller_type", "email", "country", "city",
        "creation_time"
    ]
    ...
    fieldsets = (
        ...
        ("Связи", {
            "fields": ("supplier_link",)
        }),
    )

    def supplier_link(self, obj):
        """Ссылка на поставщика"""
        if obj.supplied_items.exists():
            suppliers = []
            for seller_product in obj.supplied_items.all()[:5]:  # Показываем первых 5 поставщиков
                url = reverse("admin:retail_seller_change", args=[seller_product.supplier.id])
                suppliers.append(f'<a href="{url}">{seller_product.supplier.seller_title}</a>')
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).prefetch_related("supplied_items__supplier")


@admin.register(Product)
class ProductAdmin(admin.ModelAdmin):
    list_display = ["product_title", "product_model", "manufacturer", "product_launch_date"]
...
class SellerProductAdmin(admin.ModelAdmin):
    list_display = [
        "seller_link", "product_link", "supplier_link", "selling_price",
        "quantity", "debt_display", "hierarchy_level", "clear_debt_button"
    ]
    list_filter = [
        "seller__city", "seller__country", "hierarchy_level", "seller__seller_type"
    ]
    search_fields = [
        "seller__seller_title", "product__product_title", "supplier__seller_title"
    ]
    ...
    def clear_debt_view(self, request, object_id):
        ...
        try:
            ...
            self.message_user(
                request,
                f"Задолженность для '{obj}' очищена.",
                messages.SUCCESS
            )
